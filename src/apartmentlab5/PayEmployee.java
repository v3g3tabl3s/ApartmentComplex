...
/**
 * creates gui for paying employees
 * @author Owner
 */
class PayEmployee extends JFrame{
    ...
    public PayEmployee(MainMenuGui M, ShowTable empTables, ApartmentComplex A) {
        ...
        title = new JLabel("Choose the proper ID number of the employee you'd to get paid today");
        ...
        if(empSize > 0)
        {
            ...
            for (int i = 0; i < empSize; i++) {
                IDNumString[i] = "ID num "+ A.getEmployeeList().get(i).getIDnum();
        ...
        but.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(empSize == 0)
                {
                   ...
                   PayEmployee newGui = new PayEmployee(menu,TT,AptComplex);
                   dispose();
                }
                else
                {   
                    ...
                    String output = "Would you like "+AptComplex.getEmployeeList().get(num).getFirstName()+" "+AptComplex.getEmployeeList().get(num).getLastName()+" to be paid?";
                    int n = JOptionPane.showConfirmDialog(null, output,"Message",JOptionPane.OK_CANCEL_OPTION);
                    
                    if(n == JOptionPane.OK_OPTION)
                    {
            
                        if(AptComplex.payAnEmployee(IDNums[num]))
                        {
                            JOptionPane.showMessageDialog(null, "Not enough money in the operating Budget\nOperating Budget is: $" +df.format(AptComplex.getBalance()));
                        }
                        else
                        {
                            JOptionPane.showMessageDialog(null, AptComplex.getEmployeeList().get(num).getFirstName()+" "+AptComplex.getEmployeeList().get(num).getLastName()+" has been paid.\nOperating Budget is at: $"+df.format(AptComplex.getBalance()));
                        }
        ...
        try{
        BufferedImage kittens = ImageIO.read(new File("moneyCat.jpg"));
